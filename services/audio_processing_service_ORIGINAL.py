...
class AudioProcessingService:
    ...
    def process_audio_file(audio_path: str, state: CompositionStateDTO) -> Dict[str, Any]:
        ...
        # Apply filter BEFORE exponent (critical pipeline order)
        if state.audio_processing and state.audio_processing.apply_filter:
            filter_amt = state.audio_processing.filter_amount
            min_binned = AudioProcessingService.filter_data(min_binned, filter_amt)
            max_binned = AudioProcessingService.filter_data(max_binned, filter_amt)
    ...
    def analyze_and_optimize(
        ...
        """Grid search to find optimal amplitude exponent for visual interest."""
        if mode == "speech":
        ...
        _, max_b = AudioProcessingService.bin_amplitudes(resampled, num_slots, binning_mode)
        clean_data = AudioProcessingService.filter_data(max_b, filter_amount)

        rec_threshold = -40
        if mode == "speech":
            abs_samples = np.abs(resampled)
        ...
        for exp in candidates:
            ...
            # Immutable log accumulation
            logs = logs + [f"Exp {exp}: Score={score:.3f}"]
            if score > best_score:
                best_score = score
                best_exp = exp
