...
**Generated on:** 2025-12-22 16:19:09
...
DIAGNOSTIC VERSION - Adds JSON output for parity comparison with web app.
...
class AudioPipelineService:
    ...
    def process_pipeline(
        ...
        # --- PARITY DIAGNOSTIC: Stage 1 ---
        print(f"\n[PARITY-PYQT] stage1_resampled: {json.dumps({'count': len(raw_amplitudes), 'min': float(np.min(raw_amplitudes)), 'max': float(np.max(raw_amplitudes)), 'first10': raw_amplitudes[:10].tolist(), 'sum': float(np.sum(raw_amplitudes))})}")
        ...
        # --- PARITY DIAGNOSTIC: Stage 2 ---
        print(f"[PARITY-PYQT] stage2_binned: {json.dumps({'count': len(max_normalized), 'min': float(np.min(max_normalized)), 'max': float(np.max(max_normalized)), 'first10': max_normalized[:10].tolist(), 'last5': max_normalized[-5:].tolist(), 'sum': float(np.sum(max_normalized)), 'binning_mode': str(binning_mode)})}")
        ...
        # --- PARITY DIAGNOSTIC: Stage 3 ---
        print(f"[PARITY-PYQT] stage3_filtered: {json.dumps({'count': len(max_filtered), 'min': float(np.min(max_filtered)), 'max': float(np.max(max_filtered)), 'first10': max_filtered[:10].tolist(), 'apply_filter': request.apply_filter, 'filter_amount': request.filter_amount})}")
        ...
        # --- PARITY DIAGNOSTIC: Stage 4 (after exponent) ---
        print(f"[PARITY-PYQT] stage4_afterExponent: {json.dumps({'count': len(max_filtered), 'min': float(np.min(max_filtered)), 'max': float(np.max(max_filtered)), 'first10': max_filtered[:10].tolist(), 'exponent': request.amplitude_exponent})}")
        ...
        # --- PARITY DIAGNOSTIC: Stage 5 (scaled) ---
        print(f"[PARITY-PYQT] stage5_scaled: {json.dumps({'count': len(max_scaled), 'min': float(np.min(max_scaled)), 'max': float(np.max(max_scaled)), 'first10': max_scaled[:10].tolist(), 'last5': max_scaled[-5:].tolist(), 'sum': float(np.sum(max_scaled))})}")
        
        # --- PARITY DIAGNOSTIC: Geometry ---
        print(f"[PARITY-PYQT] geometry: {json.dumps({'max_amplitude_local': geometry.max_amplitude_local, 'true_min_radius': geometry.true_min_radius, 'center_point_local': geometry.center_point_local, 'bit_diameter': geometry.bit_diameter})}")
