...
class Config:
    ...
    # Camera settings
    # -------------------------------------------------------------------------
    # INVARIANT: FRAME_FILL determines the subject size across ALL render views.
    # 0.75 = Subject occupies 75% of the vertical frame.
    # DO NOT use arbitrary multipliers (e.g., distance * 2.0) in camera setup.
    # ALL views must derive distance using the standard FOV formula below.
    # -------------------------------------------------------------------------
    FRAME_FILL = 0.85
...
def create_semicircle_mesh(name: str, radius: float, thickness: float, 
    ...
    # Angle range: right half = -90° to 90°, left half = 90° to 270°
    if is_right:
...
def cut_slots_from_section(section: bpy.types.Object, slots: list, 
    ...
    for slot in slots:
        ...
        # Calculate Y centroid if 'y' key missing
        if 'y' in slot: 
            slot_center_y = slot['y'] - center_y
        else: 
            slot_center_y = ((slot['vertices'][0][1] + slot['vertices'][2][1])/2) - center_y
        ...
        if panel_config['number_sections'] == 2:
            ...
            if section_index == 0 and slot_center_x > 0:
                ...
                section_slots.append(slot)
        elif panel_config['number_sections'] == 4:
            # 0=TR, 1=BR, 2=BL, 3=TL
            if section_index == 0 and slot_center_x > 0 and slot_center_y > 0: section_slots.append(slot)
            elif section_index == 1 and slot_center_x > 0 and slot_center_y < 0: section_slots.append(slot)
            elif section_index == 2 and slot_center_x < 0 and slot_center_y < 0: section_slots.append(slot)
            elif section_index == 3 and slot_center_x < 0 and slot_center_y > 0: section_slots.append(slot)
        else:
            section_slots.append(slot)
...
def create_wood_material(species: str, name: str, grain_direction: str = 'vertical',
    ...
    """
    Create a PBR wood material using Object Coordinates with tiling prevention.
    """
    ...
    # --- GRAIN ROTATION ---
    grain_angles = {'horizontal': 0, 'vertical': 90}
    
    # Calculate geometric diagonal angle based on aspect ratio
    # This ensures grain aligns with the hypotenuse for non-square panels
    slope_rad = math.atan(panel_height / panel_width)
    slope_deg = math.degrees(slope_rad)
    
    # Parallel to hypotenuse (matches slope)
    # TR(-), BR(+), BL(-), TL(+) relative to horizontal
    # We map 0-180 range: TR(180-slope), BR(slope)
    ang_pos = slope_deg
    ang_neg = 180.0 - slope_deg
    
    # Pattern angles (TR, BR, BL, TL)
    radiant_angles = [ang_neg, ang_pos, ang_neg, ang_pos] # Parallel to Edge
    diamond_angles = [ang_pos, ang_neg, ang_pos, ang_neg] # Perpendicular to Edge
    
    if grain_direction == 'radiant':
        # Apply to ALL shapes (creates Starburst for n=4, V-match for n=2)
        grain_rotation_deg = radiant_angles[section_index % 4]
    elif grain_direction == 'diamond':
        # Apply to ALL shapes
        grain_rotation_deg = diamond_angles[section_index % 4]
    else:
        # Standard Linear
    ...
    # --- SCALE & OFFSET (FIXED FOR TILING) ---
    # Texture Size: 400cm (4m). World Unit: 1m.
    # Scale = 1.0 / 4.0 = 0.25 maps 1 UV unit to 4 meters.
    tex_scale = 0.25 
    mapping.inputs['Scale'].default_value = (tex_scale, tex_scale, 1.0)
    
    # Tiling Prevention:
    # Panel max width ~2.2m. In UV space: 2.2m * 0.25 = 0.55.
    # We have 1.0 - 0.55 = 0.45 "slack" space.
    # We center at 0.5, allowing a max jitter of +/- 0.225 before hitting an edge.
    import random
    random.seed(section_index + hash(species))
    safe_margin = 0.2 # Safe random jitter range
    ...
    if diffuse_file:
        ...
        diffuse_tex.extension = 'REPEAT' # Safe now because we constrain UVs to [0,1]
        ...
        principled.inputs['Base Color'].default_value = (0.4, 0.25, 0.15, 1)
...
def setup_hdri_lighting():
    """Setup HDRI environment lighting with robust fallback."""
    # Get or create world
    ...
    # Environment texture (with fallback)
    hdri_loaded = False
    if Config.HDRI_PATH.exists():
        try:
            ...
            hdri_loaded = True
            print(f"Loaded HDRI: {Config.HDRI_PATH}")
        except Exception as e:
            print(f"Failed to load HDRI image: {e}")
    else:
        print(f"HDRI not found at {Config.HDRI_PATH}")

    if not hdri_loaded:
        print("Using default Grey lighting.")
    ...
    # --- SURGICAL FIX: Disable shadow casting ---
    # This keeps the glossy highlights on the wood but removes the shadow on the wall
...
def setup_camera(panel_config: dict, view: str = 'wall', 
                 focus_point: tuple = (0.0, 0.0), 
                 azimuth: float = 0.0, 
                 elevation: float = 0.0, 
                 zoom: float = 4.0):
    """
    Setup camera for specified view type.
    focus_point: (x, y) in inches.
    azimuth: Rotation around vertical axis (deg).
    elevation: Tilt down angle (deg).
    zoom: Magnification factor (1.0 = fill frame).
    """
    ...
    if view == 'wall':
        ...
        # 3/4 Gallery View (Right Side) - Eye Level
        cam_data.type = 'PERSP'
        cam_data.lens = 50
        
        # Use SSOT for distance (Consistent size with Wall view)
        dist = get_camera_distance(max_dimension, cam_data.lens)
        
        # Position: 30-degree angle from Right, Z=0 (Eye Level)
        angle_rad = math.radians(30)
        x_pos = math.sin(angle_rad) * dist
        y_pos = -math.cos(angle_rad) * dist # Negative Y is 'Front'
        
        cam_obj.location = (x_pos, y_pos, 0)
        ...
        # Detail view - Orbit Target
        cam_data.type = 'PERSP'
        cam_data.lens = 50
        
        # 1. Calculate Distance based on Zoom Factor
        # Zoom 1.0 = Full Frame. Zoom 4.0 = 1/4 distance.
        full_dist = get_camera_distance(max_dimension, cam_data.lens)
        dist = full_dist / zoom
        
        # 2. Define Target in World Space (Meters)
        # Panel is Rotated (90,0,0) -> Panel X=World X, Panel Y=World Z, Thickness=World -Y
        t_x = focus_point[0] * Config.SCALE_FACTOR
        t_z = focus_point[1] * Config.SCALE_FACTOR
        t_y = -thick_m # Surface of wood facing camera
        target_vec = Vector((t_x, t_y, t_z))
        
        # 3. Calculate Camera Position via Spherical Coordinates
        # Azimuth 0 = Front (-Y direction). 
        # Elevation 0 = Level. 90 = Top-down.
        rad_az = math.radians(azimuth)
        rad_el = math.radians(elevation)
        
        # Spherical conversion (Y is depth axis here)
        # x = dist * sin(az) * cos(el)
        # y = -dist * cos(az) * cos(el) (Negative because -Y is front)
        # z = dist * sin(el)
        offset_x = dist * math.sin(rad_az) * math.cos(rad_el)
        offset_y = -dist * math.cos(rad_az) * math.cos(rad_el)
        offset_z = dist * math.sin(rad_el)
        
        cam_obj.location = target_vec + Vector((offset_x, offset_y, offset_z))
        
        # 4. Track To Target
        direction = target_vec - cam_obj.location
        # -Z is camera forward axis, Y is camera up axis
...
def setup_animated_camera(panel_config: dict, frames: int = 90, is_turntable: bool = False, view_mode: str = 'wall'):
    ...
    if view_mode == 'close':
        ...
        # Use SSOT for distance
        # panel_diameter serves as the bounding dimension here
        distance = get_camera_distance(panel_diameter, cam_data.lens)
...
def create_circular_quadrant_mesh(name: str, radius: float, thickness: float, 
                                  center_x: float, center_y: float, quadrant: int) -> bpy.types.Object:
    """
    Create a 90-degree pie slice for circular n=4.
    ...
    # Angles: TR(0-90), BR(270-360), BL(180-270), TL(90-180)
    if quadrant == 0: start, end = 0, math.pi/2
    elif quadrant == 1: start, end = -math.pi/2, 0
    elif quadrant == 2: start, end = math.pi, 3*math.pi/2
    elif quadrant == 3: start, end = math.pi/2, math.pi
    
    center_b = bm.verts.new((0, 0, 0))
    center_t = bm.verts.new((0, 0, thickness))
    ...
    try: bm.faces.new([center_b] + verts_bottom[::-1] if quadrant in [0,3] else [center_b] + verts_bottom)
    except: pass
    try: bm.faces.new([center_t] + verts_top if quadrant in [0,3] else [center_t] + verts_top[::-1])
    except: pass
    for i in range(segments):
        try: bm.faces.new([verts_bottom[i], verts_bottom[i+1], verts_top[i+1], verts_top[i]])
        except: pass
    try: bm.faces.new([center_b, verts_bottom[0], verts_top[0], center_t])
    except: pass
    try: bm.faces.new([center_b, center_t, verts_top[-1], verts_bottom[-1]])
    except: pass
...
def create_backing_meshes(backing_params: dict, panel_config: dict) -> list:
    ...
    2. Cut with plane at x = ±inset (not x=0) for flat edge inset
    ...
    if num_sections == 2:
        ...
        for idx in range(2):
            ...
            # Cutter edge at x = ±inset instead of x = 0
            ...
            # - Flat edge is at x = ±inset in local coords
            if is_right:
        ...
        # Circular 4-part backing
        offset_val = (separation / 2)
        # Quadrants: 0=TR, 1=BR, 2=BL, 3=TL
        quad_mult = [(1, 1), (1, -1), (-1, -1), (-1, 1)]
        ...
        for idx in range(4):
            ...
            bpy.ops.mesh.primitive_cylinder_add(
                radius=backing_radius, depth=backing_thickness, vertices=64, location=(0,0,z_pos)
            )
            backing = bpy.context.active_object
            backing.name = f'backing_{idx}'
            
            # Cutters for straight-edge insets
            cs = backing_radius * 3
            
            # Cutter 1: X-axis cut
            cx = inset - (cs/2) if mx > 0 else -inset + (cs/2)
            bpy.ops.mesh.primitive_cube_add(size=cs, location=(cx, 0, z_pos))
            c1 = bpy.context.active_object
            
            # Cutter 2: Y-axis cut
            cy = inset - (cs/2) if my > 0 else -inset + (cs/2)
            bpy.ops.mesh.primitive_cube_add(size=cs, location=(0, cy, z_pos))
            c2 = bpy.context.active_object
            
            # Apply cuts
            for cutter in [c1, c2]:
                mod = backing.modifiers.new('cut','BOOLEAN')
                mod.object = cutter
            ...
            # Position backing piece
            backing.location.x = mx * offset_val
            backing.location.y = my * offset_val
            backing_meshes.append(backing)
...
def rotate_panel_to_wall(sections: list):
    """
    Rotate panel and texture space to vertical wall orientation.
    Fixes the vertical streak issue by rotating the projection space.
    """
    print("Rotating panel to wall orientation (Vertical)...")
    ...
    # 2. Rotate the Texture Space (CRITICAL FIX)
    # Because we use Object coordinates, the reference object must rotate with the panel
    # otherwise the texture projects sideways through the mesh (creating streaks).
    coord_empty = bpy.data.objects.get('TextureCoordOrigin')
    if coord_empty:
        coord_empty.rotation_euler = (math.radians(90), 0, 0)    
...
def build_panel_from_json(json_data: dict) -> list:
    ...
    if panel_config['shape'] == 'circular':
        if panel_config['number_sections'] == 2:
            ...
            # Circular 4 sections (Pie slices)
            radius = panel_config['outer_radius']
            thickness = panel_config['thickness']
            separation = panel_config['separation']
            
            offset_val = (separation / 2) * Config.SCALE_FACTOR
            ...
            for idx in range(4):
                ...
                section = create_circular_quadrant_mesh(
                    f"section_{idx}",
                    radius * Config.SCALE_FACTOR,
                    thickness * Config.SCALE_FACTOR,
                    mx * offset_val,
                ...
                # Apply material
                mat_idx = idx if idx < len(section_materials) else 0
                if idx < len(section_materials):
                    species = section_materials[idx].get('species', 'maple')
                else:
                    species = 'maple'
                ...
                # Pass dimensions for proper radiant calculation
                mat = create_wood_material(species, f"wood_{species}_{idx}", grain_direction, idx, 'circular',
                                           panel_config['finish_x'], panel_config['finish_y'])
                section.data.materials.append(mat)
...
def parse_args():
    ...
    while i < len(argv):
        if argv[i] == '--config':
            ...
            # Format: "x,y" e.g., "6.0,0.0"
            parts = argv[i + 1].split(',')
            args['focus'] = (float(parts[0]), float(parts[1]))
            i += 2
        elif argv[i] == '--azimuth':
            # 0=Front(6oc), 90=Right(3oc), -90=Left(9oc)
            args['azimuth'] = float(argv[i + 1])
            i += 2
        elif argv[i] == '--elevation':
            # 0=EyeLevel, 90=TopDown
            args['elevation'] = float(argv[i + 1])
            i += 2
        elif argv[i] == '--zoom':
            # Magnification factor (1.0 = Full Frame)
            args['zoom'] = float(argv[i + 1])
            i += 2
        else:
            i += 1
...
def main():
    ...
    for v in views:
        ...
        # Pass manual controls (defaults: Focus=Center, Azimuth=0(Front), Elev=0(Level), Zoom=4x)
        focus_point = args.get('focus', (0.0, 0.0))
        az = args.get('azimuth', 0.0)
        el = args.get('elevation', 0.0)
        zm = args.get('zoom', 4.0)
        
        setup_camera(panel_config, v, focus_point, az, el, zm)
