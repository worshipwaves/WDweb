...
def setup_camera(meshes: list, view: str = 'wall'):
    ...
    if view == 'wall':
        ...
        cam_obj.location = (center.x + distance * 0.7, center.y - distance * 0.7, center.z + distance * 0.0)
...
def setup_turntable(meshes: list, frames: int = 72):
    """
    Panel rotates 360° on vertical axis. Camera fixed.
    Outputs PNG sequence with transparency.
    """
    # Get panel bounds
    min_coord = Vector((float('inf'), float('inf'), float('inf')))
    max_coord = Vector((float('-inf'), float('-inf'), float('-inf')))
    ...
    center = (min_coord + max_coord) / 2
    max_dimension = max(max_coord.x - min_coord.x, max_coord.z - min_coord.z)
    
    # Create parent empty for rotation
    panel_parent = bpy.data.objects.new('PanelRotator', None)
    panel_parent.location = (center.x, 0, center.z)
    bpy.context.collection.objects.link(panel_parent)
    
    # Parent all panel meshes to rotator
    for mesh in meshes:
        if mesh.name.startswith('section_') or 'backing' in mesh.name.lower():
            mesh.parent = panel_parent
            # Adjust location relative to parent
            mesh.location = mesh.location - panel_parent.location
    ...
    cam_obj.location = (center.x, -distance, center.z)
    
    direction = Vector((center.x, 0, center.z)) - cam_obj.location
    cam_obj.rotation_euler = direction.to_track_quat('-Z', 'Y').to_euler()
    ...
    panel_parent.rotation_euler = (0, 0, 0)
    panel_parent.keyframe_insert(data_path='rotation_euler', frame=1)
    
    panel_parent.rotation_euler = (0, math.radians(360), 0)
    panel_parent.keyframe_insert(data_path='rotation_euler', frame=frames + 1)

    # Linear interpolation for constant rotation speed
    if panel_parent.animation_data and panel_parent.animation_data.action:
        for fc in panel_parent.animation_data.action.fcurves:
            for kp in fc.keyframe_points:
                kp.interpolation = 'LINEAR'

    print(f"  Turntable: {frames} frames, 360° rotation")
    return cam_obj
...
def main():
    ...
    # === TURNTABLE MODE ===
        ...
        # No wall for turntable - transparent background
        setup_turntable(meshes, turntable_frames)
        
        turntable_output = output_file.parent / f"{output_file.stem}_turn_"
    ...
    # === STILL MODES ===
        ...
        # Wall for front/orthogonal, no wall for reverse/closeup
        if view in ('wall', 'orthogonal'):
            create_environment_wall()
        
        # Camera setup based on view
        if view == 'reverse':
